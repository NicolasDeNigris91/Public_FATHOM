...
> Espaço pra observações persistentes do mentor sobre fraquezas detectadas, módulos a revisitar, padrões de erro recorrentes. Atualizar após cada portão falhar.

(vazio)

---

## Histórico de Portões (audit log)

> Registro append-only de cada portão tentado. Formato: `YYYY-MM-DD | módulo | portão | resultado | nota`.
...
> Registro de re-tests programados (§12 do STUDY-PROTOCOL). Formato: `YYYY-MM-DD agendado | módulo | tipo (30d/90d/180d/365d) | status (pendente/passou/refresh)`.
...
## Journal de Descobertas

> Lições não-óbvias por módulo (§16 do STUDY-PROTOCOL). 3-5 frases por módulo após `done`. Não confundir com diary; é registro técnico do que surpreendeu.
...
> Sua máquina, ambiente e tooling. Cada portão prático precisa disso pra reproduzir. Documentar uma vez evita arrumação repetida e facilita o mentor entender contexto técnico.
>
...
### OS / shell
- **OS**: (macOS / Linux distro / Windows + WSL2).
- **Shell**: (bash / zsh / fish + plugins relevantes).
- **Terminal**: (iTerm2 / Alacritty / Wezterm / Windows Terminal).
- **Multiplexer**: (tmux / zellij / nenhum).

### Editor / IDE
- **Editor principal**: (VS Code / Neovim / JetBrains).
- **Extensões críticas**: (TypeScript, Rust analyzer, ESLint, etc.).
- **Configs / dotfiles**: (link pra repo se versionado).

### Linguagens / runtimes (versões fixas)
- **Node**: (versão exata, gerenciada por nvm/fnm/volta).
- **Python**: (versão + pyenv/asdf).
- **Rust**: (rustup channel).
- **Go**: (versão).
- **Bun / Deno**: (se usa).
...
### Browsers
- **Principal pra dev**: (Chrome / Firefox / Edge / Safari).
- **Secundário pra teste cross-browser**.

### Cloud / hosting
- **Conta pessoal Railway / Vercel / Netlify / Cloudflare**.
- **AWS / GCP / Azure** (se aplicável).
...
- **Anki**: (deck path, sync ativa).
- **Notas / second brain**: (Obsidian / Notion / plain markdown).

### Acesso a referências
- **Livros físicos** (lista breve dos relevantes que possui em estante):
- **Subscriptions ativas** (livros / cursos / journals):

(Preencha quando setup estabilizar. Atualize a cada mudança de máquina/major OS.)
