...
stage: professional
prereqs: [P01, P02, P03, P04, P05, P06, P07, P08, P09, P10, P11, P12]
status: locked
gates:
...
Logística v1 (Apprentice) provou que você integra um stack full-stack. v2 (Professional) prova que você opera. Sair de "funciona" pra "fica vivo sob carga, é deployado com confiança, é observado, é seguro, é caché, é rápido, é portável" é o salto entre quem ship feature e quem mantém produto.

Isso força você a integrar:
- **P01**: test strategy real, mutation testing, load tests.
- **P02-P03**: containerizado em K8s (ou Fargate) com hardening.
- **P04**: pipeline end-to-end com canary, GitOps.
- **P05-P06**: AWS via IaC reproduzível.
- **P07**: instrumentado com SLOs, dashboards, alertas baseado em burn rate.
- **P08**: hardening AppSec aplicado, threat model, scanning.
- **P09-P10**: perf otimizado em frente e backend, sob método.
- **P11-P12**: serviços auxiliares Go/Rust quando vencem; Wasm onde encaixa.
...
- **Routing inteligente**: serviço dedicado (P11 Rust) calcula rotas otimizadas; preview client-side via Wasm.
- **Webhook ingestion isolada**: serviço Go (P11) recebe e enfileira eventos externos.
...
Mas: **não adicione features que não dependam dos módulos do Professional**. Foco é operação, não scope creep.
...
- Toda infra via OpenTofu/Terraform (P06).
...
- Pipeline P04 inteiro: PR (lint, test, scan, build), main (build, sign, deploy staging), prod (manual approval, canary).
