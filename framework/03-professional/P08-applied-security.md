...
Vimos em A13. Em apps API com JWT em header, CSRF é menor preocupação. Em cookie-based, mitigue (SameSite, CSRF tokens).
...
3. **Auth hardening**:
   - Auditoria do A13: cobre todas threats (replay, IDOR, mass assignment, brute force, CSRF, JWT armadilhas)?
...
- Liga com **A13** (auth): tudo de auth aqui é foundational.
- Liga com **A09** (Postgres): RLS, prepared statements, schema design defensivo.
- Liga com **N03** (network): TLS, certs, network policies.
- Liga com **P02** (Docker): hardening, scan, SBOM.
- Liga com **P03** (K8s): NetworkPolicy, RBAC, PodSecurity.
- Liga com **P04** (CI/CD): SAST/SCA gates, secret scanning.
- Liga com **P06** (IaC): policy-as-code, drift = audit.
- Liga com **P07** (observability): security alerts, audit logs.
- Liga com **S04** (resilience): rate limit, circuit breaker contra ataques.
- Liga com **S12** (tech leadership): security culture, postmortems.
