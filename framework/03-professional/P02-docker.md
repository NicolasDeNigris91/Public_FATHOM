...
- App deve ter handler. Vimos em A07.
...
Kubernetes: prod escalável, multi-host, scheduler, service discovery, secrets, autoscaling. Vimos em P03.
...
- Liga com **N02** (OS): namespaces, cgroups, processes, signals.
- Liga com **N10** (CLI): docker como CLI; shell scripts pra build/run.
- Liga com **A05** (Next): standalone output.
- Liga com **A07** (Node): PID 1, SIGTERM handling, graceful shutdown.
- Liga com **P03** (K8s): mesma image roda em pods.
- Liga com **P04** (CI/CD): build em CI, push pra registry.
- Liga com **P05** (AWS): ECR, ECS, EKS, App Runner consumir images.
- Liga com **P08** (security): scanning, hardening, supply chain.
- Liga com **P10** (perf backend): image size = cold start em serverless.
