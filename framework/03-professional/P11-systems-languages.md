...
- **Rust ↔ Python**: PyO3.
- **Rust → WASM** (P12): `wasm-bindgen`, `wasm-pack`. Native pra browser.
...
2. **Endpoints**:
   ...
   - Persiste em tabela `external_events` (mesma do A12 ou Postgres jsonb).
...
- Routing engine compilado pra WASM (P12 preview), rodando no front client-side.
...
- Liga com **N06** (paradigmas): tipos, sum types, ownership.
- Liga com **N07** (JS deep): contraste com Node event loop.
- Liga com **A07/A08** (Node, frameworks): conceitos análogos.
- Liga com **P02** (Docker): static binary, scratch image.
- Liga com **P05** (AWS): Lambda Go/Rust runtime, Fargate.
- Liga com **P10** (perf backend): Rust como primitive de perf onde Node não chega.
- Liga com **P12** (Wasm): Rust → Wasm.
- Liga com **S02** (messaging): Go consumers/producers de Kafka/NATS.
