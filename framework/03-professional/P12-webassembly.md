...
Adicionar **routing engine Wasm** ao Logística — reuso da P11 Rust em browser e edge.

### Especificação

1. **Compile do P11 routing engine pra Wasm**:
...
4. **Performance**:
   - Compare 3 implementações:
     ...
     - Backend chamando Rust binary (via HTTP, do P11).
     - Edge Wasm.
...
- Liga com **N06** (paradigmas): types, ADTs em Rust core.
- Liga com **A03** (Web APIs): WebAssembly API, fetch loading.
- Liga com **A05** (Next): dynamic import, edge runtime.
- Liga com **P03** (K8s): Wasm runtimes (Spin) em K8s.
- Liga com **P05** (AWS): Lambda Wasm support (limited), CloudFront Functions.
- Liga com **P11** (systems langs): Rust core compiled pra Wasm.
- Liga com **S08** (services): plug-in systems via Wasm.
- Liga com **S10** (AI): ONNX Runtime Web roda Wasm.
