...
- Static sites → S3/CloudFront, Vercel, Netlify.
...
- Liga com **N09** (Git): branch protection, signing, conventional commits.
- Liga com **P01** (testes): suite roda em CI; flakiness é problema CI.
- Liga com **P02** (Docker): build, scan, sign, push.
- Liga com **P03** (K8s): GitOps com Argo CD; canary com Argo Rollouts.
- Liga com **P05** (AWS): OIDC GH → AWS, ECR, deploy targets.
- Liga com **P06** (IaC): infra deployed via mesmo pipeline (com gates extras).
- Liga com **P07** (observability): release annotations em Grafana, DORA dashboards.
- Liga com **P08** (security): supply chain, secrets, gating em vulnerabilities.
- Liga com **S08** (services vs monolith): monorepo CI; selective deploy.
- Liga com **S12** (tech leadership): DORA, processo, deployment culture.
