...
# P17 — Accessibility Testing & Automation

## 1. Problema de Engenharia

A02 cobriu accessibility como design e implementation. Mas conhecimento sem **testing automation** vira regressão silenciosa. Dev adiciona componente novo, esquece `aria-label`, PR aprova, deploy, screen reader user reclama meses depois — pior, abandona produto sem reclamar. Sem CI gate, a11y degrada continuamente.
...
S16 (product/business): a11y é gate sutil pra B2B enterprise (procurement perguntam).
...
- Lang attribute correto (linha com A19).
...
- Liga com **A01** (HTML/CSS): semantic HTML é foundation.
- Liga com **A02** (a11y): este módulo é evolução automation.
- Liga com **A04** (React): ARIA em components, hooks (useId, useEffect cleanup).
- Liga com **A05** (Next.js): SSR + a11y, lang attribute.
- Liga com **A06/A17** (mobile): native a11y APIs.
- Liga com **A19** (i18n): lang attribute, RTL, screen reader pronunciation.
- Liga com **P01** (testing): axe integra em jest, playwright.
- Liga com **P04** (CI/CD): Lighthouse CI pipeline.
- Liga com **P09** (frontend perf): bundle size de a11y libs.
- Liga com **S05** (API design): error responses humanos pra a11y feedback.
- Liga com **S16** (product): compliance + market expansion.
