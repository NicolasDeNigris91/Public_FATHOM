...
# P14 — Graphics, Audio & Real-Time Codecs
...
`getUserMedia({video, audio})` retorna MediaStream. WebRTC (A14) usa pra peer-to-peer.
...
`sRGB` → `Display P3` → wide gamut. `<canvas color-space="display-p3">`. WebGL color spaces. HDR vídeo (HDR10, Dolby Vision) chegando.
...
4. **Vídeo de comprovação de entrega**:
   ...
   - Backend salva em S3 (MinIO local).
...
- Liga com **N03** (networking): WebRTC depende de NAT traversal, UDP.
- Liga com **N07** (JS): event loop, raf, microtasks.
- Liga com **A03** (DOM/Web APIs): Canvas, MediaStream, Web Audio APIs.
- Liga com **A14** (real-time): WebRTC, SSE.
- Liga com **P09** (frontend perf): frame budget, long tasks.
- Liga com **P12** (Wasm): codecs em Wasm (ffmpeg.wasm), DSP.
- Liga com **A06/A17** (mobile): hardware codecs, AVFoundation/MediaCodec.
- Liga com **S10** (AI/LLM): inference em GPU (compute shader, Wasm-SIMD, WebNN).
