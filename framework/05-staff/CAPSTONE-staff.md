---
module: CAPSTONE-staff
title: Staff Capstone — Specialization Track + Public Showcase
stage: staff
prereqs: [ST01, ST02, ST03, ST04, ST05, ST06]
gates:
...
Você já fez:
- ST01: 2 toys de baixo nível (build-from-scratch).
- ST02: 3 capstones de domínios distintos.
- ST03: proposta org + estudo de caso.
- ST04: paper habit + 1 paper implementado.
- ST05: stream público com 6+ posts + talk.
- ST06: 3 mentees acompanhados.
- ST07 opcional: tracker IoT.
...
Tracks recomendados:
- **A. Distributed Systems Engineer**: deep em consensus, replication, multi-region. Owner de toy DB (ST01) + Raft (ST04 implementado) + paper writing.
- **B. Platform / Infra Engineer**: K8s, observability, DevEx. Toy scheduler (ST01) + plataforma interna (Logística v2 platform team angle) + multi-cluster.
- **C. Frontend / DX Architect**: React deep, build tools, framework design. Toy compiler/runtime (ST01) + DX library (S15).
...
Output:
- Toy DB completo + toy queue (ST01).
- Raft implementation (ST04 paper).
- TLA+ specs em produção (S14).
...
Output:
- Toy scheduler / load balancer (ST01).
- Platform team proposal (ST03) implementada na Logística (or thought experiment).
...
Output:
- ST02 RAG capstone production-quality.
...
- N12 cripto deep dive blog.
- Web3 contract audit (S11 capstone).
...
- Unit economics simulator + dashboards (S16).
...
3. **Portfolio site**:
   - Próprio domain.
   - About, Projects (com cada output do framework linkado), Blog (de ST05), Talks, Contact.
   - SEO basics, performance pristine (P09), accessibility (A02).
4. **Promo case**:
...
- **Mentor mentor**: tornar mentee de ST06 que vire Senior ou Staff.
...
- Liga com **CAPSTONE-novice/apprentice/professional/senior**: tudo é fundação.
- Liga com **ST01-ST07**: outputs cristalizam aqui.
