...
status: locked
quiz:
  - q: "Por que `for_each` é preferido a `count` em recursos Terraform iterados?"
    options:
      - "for_each é mais rápido em plan time"
      - "count usa index posicional: deletar elemento 1 de lista de 4 re-cria índices 2 e 3, catastrófico em prod; for_each com map keys estáveis evita isso"
      - "count não funciona em Terraform 1.5+"
      - "for_each suporta condicionais, count não"
    correct: 1
    explanation: "Com `count`, lista `[a, b, c, d]` mapeia para índices 0-3. Remover `b` desloca tudo, fazendo Terraform DESTRUIR e recriar c e d. `for_each` com map associa cada key a recurso específico."
  - q: "Qual a maneira correta de manejar password de RDS em IaC?"
    options:
      - "Hardcoded em .tf versionado, encryptado por GPG"
      - "Em variable.tf com default vazio"
      - "Gerar via `random_password` + store em Secrets Manager; app lê via IAM role; Terraform NUNCA tem o valor literal em código"
      - "Em arquivo .tfvars commitado mas adicionado ao .gitignore"
    correct: 2
    explanation: "Anti-pattern #1 em IaC é secret em código versionado. State pode conter; minimize. App deve assumir IAM role e ler de Secrets Manager runtime."
  - q: "Por que separar state files por blast radius (networking/data/compute)?"
    options:
      - "Reduz tamanho de cada state file melhorando performance de plan/apply"
      - "Mudança em ECS service não força re-plan da VPC; apps não precisam tocar networking; isolamento de impacto"
      - "Terraform exige state separado por provider"
      - "Permite usar diferentes versões de Terraform por layer"
    correct: 1
    explanation: "State único = mudança em S3 bucket pode forçar replan de DB. Separação por blast radius (network raro/manual, compute frequente) limita impacto e tempo de plan."
  - q: "Qual o problema de Terraform `count = length(var.subnets)` ao receber lista variável?"
    options:
      - "Não funciona com providers AWS"
      - "É index posicional; mudança na ordem ou remoção de elemento força destroy/recreate de recursos não relacionados"
      - "Performance é O(n²)"
      - "Causa drift detection falhar"
    correct: 1
    explanation: "Lista é ordenada por índice. Remover `subnets[0]` faz Terraform pensar que subnet[1] mudou de identity. Solução: `for_each = { for s in var.subnets : s.name => s }` com keys estáveis."
  - q: "O que o `moved {}` block (TF 1.1+) resolve em refactor de código IaC?"
    options:
      - "Move state file entre buckets S3"
      - "Renomear resource sem destroy/recreate, declarativamente versionado em git em vez de `terraform state mv` manual"
      - "Migra recursos entre regions AWS"
      - "Substitui `terraform import` para CRDs"
    correct: 1
    explanation: "`terraform state mv` CLI é manual e não-auditável. `moved {}` block fica versionado no Git, executa no apply, e pode ser deletado após rollout completo. Padrão moderno para refactor seguro."
---
