...
status: locked
quiz:
  - q: "Qual a diferença essencial entre Continuous Delivery e Continuous Deployment?"
    options:
      - "Delivery usa CI/CD, Deployment usa apenas CI"
      - "Em Delivery cada commit verde é deployable mas deploy é manual; em Deployment cada commit verde vai automático para prod"
      - "Delivery é só para SaaS, Deployment é para mobile"
      - "Delivery exige feature flags, Deployment não"
    correct: 1
    explanation: "Continuous Deployment exige maturidade alta: testes confiáveis, observability forte, rollback automático e feature flags. Projetos novos começam em Delivery (deploy manual em prod) e evoluem."
  - q: "Por que usar OIDC entre GitHub Actions e AWS é superior a guardar AWS access keys nos GH Secrets?"
    options:
      - "OIDC é mais rápido em latência de autenticação"
      - "OIDC elimina credenciais long-lived: runner troca GH OIDC token por credencial AWS temporária por job"
      - "OIDC reduz custo de API calls"
      - "OIDC permite cross-region deploys que keys não permitem"
    correct: 1
    explanation: "Long-lived keys vazadas comprometem indefinidamente. Com OIDC, o runner assume role temporária via federation, sem chaves estáticas armazenadas. Padrão moderno em CI/CD cloud."
  - q: "Em deploy strategy, qual a vantagem do canary com `setWeight` progressivo + analysis gate vs blue/green?"
    options:
      - "Canary não exige duas cópias da infra rodando"
      - "Canary expõe mudança a subset real de tráfego e auto-rollback baseado em métricas SLI antes de 100% ficar afetado"
      - "Blue/green tem latência maior que canary"
      - "Canary suporta DB schema migrations breaking"
    correct: 1
    explanation: "Canary 10%→25%→50%→100% com analysis (success rate, p99, error budget) detecta regressão real-traffic em escala parcial. Blue/green é all-or-nothing no switch — bug afeta 100% no momento do cutover."
  - q: "Qual é a métrica DORA `Lead Time for Changes` quando medida CORRETAMENTE?"
    options:
      - "Tempo do ticket aberto no Jira até deploy em prod"
      - "Tempo do commit (author.date) até deploy completar em produção"
      - "Tempo médio entre dois deploys consecutivos"
      - "Tempo total do sprint até release"
    correct: 1
    explanation: "Lead time é code commit → prod, não ideia → prod (esse é cycle time). Confundir leva métrica vanity que não muda comportamento e não reflete eficiência real do pipeline."
  - q: "Por que Trunk-based + feature flags é dominante em 2026 vs GitFlow para SaaS?"
    options:
      - "Trunk-based dispensa code review"
      - "Decoupling de deploy e release: ship dark com flag OFF, ativa quando pronto, reverte sem redeploy"
      - "Trunk-based torna desnecessário ter staging environment"
      - "GitFlow não suporta monorepos"
    correct: 1
    explanation: "Feature flags separam ato de fazer deploy do ato de liberar funcionalidade. Trunk-based + flags + deploys frequentes elimina merge hell de release branches longas e dá rollback instantâneo via toggle."
---
