...
Estes são algoritmos clássicos (N05) com implementações otimizadas pra graph storage.
...
Confusão comum. GraphQL é **API query language** (S05), executada por resolvers que falam com qualquer storage (incluindo SQL). Graph DB é **storage**. GraphQL pode usar graph DB ou não.
...
- Liga com **N04** (data structures): adjacency list/matrix.
- Liga com **N05** (algorithms): Dijkstra, A\*, BFS, PageRank, community detection.
- Liga com **A09** (Postgres): comparação direta com `WITH RECURSIVE` + AGE.
- Liga com **A15** (search): híbrido graph + full-text (busca contextual).
- Liga com **P10** (backend perf): hop count = profile target.
- Liga com **S05** (API design): GraphQL ≠ Graph DB; diferenciação útil.
- Liga com **S06** (DDD): aggregates podem virar nodes; modelagem mais expressiva.
- Liga com **CAPSTONE-apprentice** e **CAPSTONE-senior**: roteamento real.
