...
Diferente de real-time em-app. Push notifications via APN (iOS) e FCM (Android) — chegam mesmo com app fechado. Cobertos no A06.
...
- Liga com **N03** (networking): TCP, HTTP upgrade, TLS, NAT.
- Liga com **A03** (Web APIs): EventSource, WebSocket browser.
- Liga com **A05** (Next): Route handlers podem servir SSE.
- Liga com **A07** (Node): event loop e socket handling.
- Liga com **A08** (frameworks): plugin Fastify-WS, Hono WebSocket adapters.
- Liga com **A11** (Redis): fan-out, Streams pra replay.
- Liga com **A13** (auth): WS upgrade auth.
- Liga com **P03** (K8s): sticky sessions, ingress config.
- Liga com **P05** (AWS): API Gateway WebSocket, Lambda limitations.
- Liga com **S02** (messaging): Kafka como backbone pra fan-out de larga escala.
- Liga com **S04** (resilience): connection retries, circuit breaker dependendo de Redis.
