...
- Liga com **A13** (auth): identity de Stripe Connect accounts; OAuth.
- Liga com **A09** (Postgres): ledger imutável, transactions, MVCC.
- Liga com **A14** (real-time): atualizar UI quando webhook chega (push notification, socket).
- Liga com **N12** (crypto): webhook signature verification (HMAC).
- Liga com **P08** (security): PCI-DSS, secret management.
- Liga com **P07** (observability): payment latency, success rate, webhook backlog.
- Liga com **S03** (event-driven): webhooks são eventos; outbox + idempotency é exatamente este pattern.
- Liga com **S04** (resilience): retry policy, dunning, circuit breaker no PSP.
- Liga com **S16** (product/business): pricing, unit economics, take rate.
