...
Lighthouse a11y score é util mas mente — score 100 não significa "site acessível", significa "passou checks automáticos". Combine com manual. P17 cobre testing automation profundamente.
...
Pegue o **dashboard de logística** do A01 e leve a a11y a **AA real**.
...
- Liga com **A01**: HTML semântico é a base. Sem ele, a11y vira ARIA gambiarra.
- Liga com **A04** (React): Headless UI libs (Radix UI, React Aria, Headless UI da Tailwind) implementam a maior parte dos APG patterns testados. Use elas em vez de reinventar.
- Liga com **A05** (Next.js): SSR ajuda a11y porque conteúdo crítico está no HTML inicial — screen reader não espera JS.
- Liga com **P09** (perf): tempo de hydration impacta acessibilidade real (controle não responde até hidratar).
- Liga com **A03** (DOM/APIs): `IntersectionObserver` pra revelar conteúdo on-scroll precisa pensar em screen reader (announcing dynamic).
