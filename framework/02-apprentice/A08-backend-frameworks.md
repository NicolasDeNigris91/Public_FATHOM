...
- Liga com **A07**: framework é camada sobre módulo `http`. Hooks e plugins refletem fases do processamento que vimos.
- Liga com **A09** (Postgres): pool de conexões compartilhada por handlers; cuidado em serverless.
- Liga com **A10** (ORMs): Prisma/Drizzle integram com qualquer framework.
- Liga com **A13** (auth): JWT middleware, session middleware, OAuth callbacks.
- Liga com **A14** (real-time): WebSocket integrations (Fastify-WS, ws + Express, Hono with @hono/websocket).
- Liga com **P01** (testes): supertest / fastify.inject / Hono test API; integration tests.
- Liga com **P07** (observability): structured logging, metrics endpoints, tracing instrumentation.
- Liga com **P08** (security): helmet, CORS, rate limit, input validation.
- Liga com **S05** (API design): REST maturity, versioning, pagination, error formats.
