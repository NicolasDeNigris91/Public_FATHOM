...
**PerformanceObserver**: notifica eventos de performance (LCP, FID, CLS, long tasks, fetch entries). Pra observabilidade frontend. Veremos mais em [P09](../03-professional/P09-frontend-performance.md).
...
- Liga com **N07**: Web APIs disparam eventos no event loop. `requestAnimationFrame` é macrotask especial. Microtasks (Promise) precedem rAF.
- Liga com **A04** (React): React abstrai DOM mas eventos sintéticos têm comportamento próprio (event pooling histórico, delegação no root). Refs vazam pra DOM real. Effects rodam após paint.
- Liga com **A05** (Next): Service Worker em RSC tem nuances de hydration. PWA com Next exige plugin (`next-pwa`).
- Liga com **A07** (Node): Node moderno tem `fetch`, `URL`, `WebStreams`, `AbortController` — mesma API. Reuso de conhecimento.
- Liga com **P09** (perf): observers + DevTools Performance + Web Vitals dependem deste módulo.
