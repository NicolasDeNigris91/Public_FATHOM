...
- Liga com **N07** (JS deep): hooks dependem de closures. Effects rodam em microtasks ou após paint. `Object.is` é base de comparação shallow.
- Liga com **N08** (TS): tipagem de hooks com generics, `Children` tipos, polymorphic components.
- Liga com **A03** (DOM): React abstrai DOM mas vaza. Refs, eventos sintéticos, focus.
- Liga com **A05** (Next.js): App Router é puramente RSC + Client. Caching layers do Next interagem com Suspense.
- Liga com **A06** (React Native): mesmas APIs de React (hooks, suspense), renderer diferente. Conhecimento transfere.
- Liga com **P09** (perf): Core Web Vitals impactados por hydration cost, RSC payload, code splitting.
