...
# A19 — Internationalization & Localization (i18n / l10n)
...
i18n não é "tradução do front-end". Inclui: Unicode correctness (UTF-8/16, normalization, combining chars, emoji), pluralization (10+ regras em russo, polonês, árabe), gender-aware messages, number/currency/date format por locale, timezone math, RTL layouts, fonts com glyphs corretos, sorting/search com collation, input methods, certificados e regulamentação por país (LGPD, GDPR, CCPA), tax calc por região (já em A18), nomes pessoais cross-cultural (não há "first name + last name" universal), endereços com formatos divergentes.
...
Elasticsearch / Meilisearch (A15): tokenizer + stemmer language-aware. RSLP (português) ≠ Snowball English.
...
- Liga com **N12** (cripto): Unicode normalization affects hashing.
- Liga com **A01** (HTML/CSS): logical properties, lang.
- Liga com **A02** (a11y): lang attribute, RTL accessibility.
- Liga com **A04** (React): useTranslation, formatting.
- Liga com **A05** (Next.js): i18n routing.
- Liga com **A09** (Postgres): collation, citext, unaccent.
- Liga com **A15** (search): tokenizer per language.
- Liga com **A18** (payments): currency formatting.
- Liga com **P08** (security): unicode normalization in input validation (homoglyph attacks).
- Liga com **P17** (a11y testing): lang correctness checked.
- Liga com **S05** (API design): Accept-Language header, language negotiation.
- Liga com **S16** (product): mercados internacionais drive feature.
