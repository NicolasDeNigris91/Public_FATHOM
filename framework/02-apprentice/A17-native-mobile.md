...
Construir **app nativo iOS + Android** que consome a API da Logística (do CAPSTONE-apprentice).
...
- Liga com **N02** (OS): Android = Linux. iOS = Darwin.
- Liga com **N03** (networking): TLS, pinning, HTTP/2 (APNs).
- Liga com **N07** (JS): RN bridge — agora você sabe o que está abaixo.
- Liga com **A06** (RN): contraste arquitetural.
- Liga com **A13** (auth): OAuth2 mobile-specific (PKCE obrigatório).
- Liga com **A14** (real-time): WebSocket em mobile, reconnect agressivo, energy.
- Liga com **N12** (crypto): Keychain/Keystore baseiam em hardware secure enclave.
- Liga com **P08** (security): mobile threats — root/jailbreak detection, anti-tamper.
- Liga com **P09** (perf frontend): Core Web Vitals → mobile equivalent (energy, frame time).
