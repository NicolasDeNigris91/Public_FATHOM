...
- Mudanças destrutivas (DROP COLUMN, RENAME) requerem dual-write ou janelas de manutenção. Vimos em A09.
...
Reescrever **Logística API** sobre ORM com tipagem forte, mantendo o schema do A09.

### Especificação

1. **Stack**:
   - Node 22 + Fastify (do A08).
   - Drizzle ORM como principal, com Drizzle Kit pra migrations.
   - Postgres 16 (mesmo schema do A09 — agora gerado por Drizzle).
2. **Schema**:
   - Reproduzir o schema do A09 em `schema.ts`.
   - Migrations geradas via `drizzle-kit generate`.
   - Comparar SQL gerado com o que você escreveu no A09 manualmente.
3. **Endpoints com Drizzle**:
   - Mesmos endpoints do A08 — agora todas queries via Drizzle.
...
- README documenta:
  ...
  - Comparação migration declarativa (Drizzle) vs imperativa (SQL puro do A09).
...
- Liga com **A09** (Postgres): você só usa ORM bem se entende o que ele gera.
- Liga com **A08** (frameworks): integration via plugins, lifecycle, error mapping.
- Liga com **A11** (Redis): cache de queries pesadas; invalidate ao escrever.
- Liga com **A13** (auth): row-level security policies em Postgres + ORM.
- Liga com **P01** (testes): integration test patterns, Testcontainers.
- Liga com **P04** (CI/CD): migration step antes de deploy.
- Liga com **P10** (perf): EXPLAIN sobre SQL gerado.
- Liga com **S03** (event-driven): outbox pattern: ORM escreve em domain table + outbox table na mesma txn.
