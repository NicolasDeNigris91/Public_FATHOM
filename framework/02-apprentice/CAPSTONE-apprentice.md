...
stage: apprentice
prereqs: [A01, A02, A03, A04, A05, A06, A07, A08, A09, A10, A11, A12, A13, A14]
status: locked
gates:
  pratico: { status: pending, date: null, attempts: 0, notes: null }
---

# CAPSTONE Apprentice — Logística v1

## 1. Por que esse capstone existe

Ao final do Apprentice, você passou por 14 módulos com desafios isolados. Cada um arranhou um pedaço da Logística (web, mobile, backend, DB, cache, auth, real-time). O capstone **junta tudo num produto coerente** — multi-tenant real, deployável em Railway, usado por três personas (lojista, courier, cliente). É a primeira vez que você ship um sistema fim-a-fim.

Isso força você a integrar:
- **A01-A05**: front-end web acessível, performante, em Next.
- **A06**: app mobile pro courier.
- **A07-A08**: backend Node estruturado.
- **A09-A10**: schema relacional + ORM disciplinado.
- **A11**: Redis pra cache, rate limit, idempotency, fan-out real-time.
- **A12**: opcional — Mongo pra eventos heterogêneos. Documente decisão de incluir ou não.
- **A13**: auth completa, MFA, RBAC, multi-tenant.
- **A14**: tracking real-time courier ↔ lojista ↔ cliente.
...
- **Front (lojista + cliente)**: Next.js 15+ App Router, TypeScript strict, Tailwind, acessível (A01-A05).
- **App (courier)**: Expo + Expo Router, Reanimated, FlashList, MMKV, SecureStore (A06).
- **Backend**: Fastify + TypeScript + Drizzle + Postgres + Redis. Hono adicional opcional pra edge functions específicas (A07-A11).
- **Auth**: própria, sem Auth0/Clerk. Senhas Argon2id, MFA TOTP, passkeys, OIDC Google. RBAC com tenant isolation (A13).
- **Real-time**: WebSocket (notifications, courier upload), SSE (tracking público), Redis pub/sub fan-out entre N instâncias (A14).
- **Deploy**: Railway. App + DB + Redis no mesmo projeto.

### 3.2 Schema (mínimo)

Reusar/expandir do A09:
...
- GPS streaming durante en route via WebSocket. Em background, app fica suspenso → ao voltar pra foreground sincroniza buffer (vimos em A06 stretch).
...
### 3.9 Testes (mínimo viável; aprofundar em P01)
...
P01 vai forçar coverage real. Aqui, plante a estrutura.
...
- "Vou usar uma biblioteca pronta de auth" — não. Você passou por A13 pra implementar.
...
- **Mongo pra eventos externos** (A12 use case): receba webhooks de PagSeguro/Mercado Pago/SendGrid, persista em Mongo, processe com worker.
