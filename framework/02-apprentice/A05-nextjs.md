...
Migrar **Logística (versão A01/A02 vanilla)** pra Next.js App Router, com cache controlado, Server Actions, e streaming.
...
1. **Setup**:
   ...
   - TypeScript strict.
   - Tailwind config compartilhado com versão A01.
2. **Rotas**:
   - `/` — landing.
...
- Sem React Query nesta fase (foco no que Next dá nativamente; A11 introduz cache externo se quiser).
...
- Liga com **A04**: RSC, Suspense, Server Actions são features do React. Next só orquestra.
- Liga com **A03** (Web APIs): cookies, headers, Web Streams.
- Liga com **A07/A08** (Node): route handlers/Server Actions rodam em Node ou Edge.
- Liga com **A09** (Postgres): queries em RSC; cuidado com pool em serverless (use serverless driver ou Postgres com connection pooling como pgBouncer/Supavisor).
- Liga com **A13** (auth): middleware com sessions.
- Liga com **P02** (Docker): `output: 'standalone'` simplifica imagem Docker.
- Liga com **P05** (AWS): hospedar Next em AWS via OpenNext, ECS, Lambda. Vercel é mais simples mas vendor lock-in.
- Liga com **P09** (perf): Next entrega Web Vitals decentes por default; você refina.
