...
# A15 — Search Engines & Information Retrieval
...
E hoje, busca semântica (vector search via embeddings) entrou no mainstream — não substitui keyword, complementa. RAG (S10) e busca híbrida BM25+vetor são padrão.
...
- **CDC** (Change Data Capture): Postgres logical replication / Debezium / outbox table → Kafka → indexer. Mais robusto, conecta com S03.
...
1. **Setup**: Meilisearch (Docker) + Postgres com pgvector (já no A09).
2. **Indexação**:
...
- Liga com **N04** (data structures): inverted index, FST, B-Tree.
- Liga com **N05** (algorithms): graph search (HNSW), priority queues.
- Liga com **A09** (Postgres): pgvector, FTS, comparação clara.
- Liga com **A11** (Redis): cache de top queries.
- Liga com **A14** (real-time): updates de busca em tempo real.
- Liga com **P07** (observability): query latency p95, zero-result rate.
- Liga com **P09** (frontend perf): autocomplete debounce, pagination.
- Liga com **S03** (event-driven): CDC + outbox.
- Liga com **S10** (AI/LLM): RAG é busca + LLM.
