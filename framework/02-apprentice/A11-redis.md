...
1. **Stack**:
   - Continuação do projeto (Fastify + Drizzle + Postgres do A08-A10).
...
3. **Padrão 2 — Rate limit distribuído**:
   - Substitua o rate limit in-memory do A08 por Redis-based (sliding window com Lua atômico).
...
- Liga com **N02** (OS): Redis é processo single-thread. Forks pra RDB. Memory-bound.
- Liga com **A07** (Node): pool de conexões `ioredis`, evt loop não bloqueia em Redis (todas comandos via socket).
- Liga com **A08** (frameworks): rate limit, session store, idempotency middleware.
- Liga com **A09** (Postgres): cache-aside on top of Postgres queries.
- Liga com **A14** (real-time): Redis pub/sub clássico pra fan-out de WebSockets.
- Liga com **P05** (AWS): ElastiCache Redis/Valkey, Memorystore (GCP).
- Liga com **P07** (observability): metrics de cache, slowlog.
- Liga com **S01/S02** (distribuídos, messaging): Streams como light queue.
- Liga com **S04** (resilience): cache stampede, circuit breaker em torno de Redis dependency.
