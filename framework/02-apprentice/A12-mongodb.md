...
1. **Stack**:
   ...
   - Driver oficial `mongodb` em Node (Fastify do A08).
...
- Liga com **A09** (Postgres): contraste claro. Mongo não substitui; complementa.
- Liga com **A10** (ORMs): Mongoose é ODM, mesma classe. Decisões de tipagem similares.
- Liga com **A11** (Redis): cache-aside funciona igual.
- Liga com **A14** (real-time): Change Streams pra fan-out reativo.
- Liga com **P03** (Kubernetes): Mongo Operator (Percona, Bitnami) pra deploy.
- Liga com **P05** (AWS): Atlas em AWS, ou DocumentDB.
- Liga com **S01** (distributed systems): replica set é Raft-ish; majority writes mapeiam quorum.
- Liga com **S03** (event-driven): Change Streams = base de CDC.
