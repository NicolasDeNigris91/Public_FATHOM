...
status: locked
quiz:
  - q: "Por que SHA-256 simples é inadequado para armazenar senhas, exigindo Argon2id/bcrypt/scrypt?"
    options:
      - "SHA-256 é facilmente reversível em milisegundos com tabelas modernas"
      - "SHA-256 é rápido demais e sem salt embutido: atacantes brute-forçam bilhões de hashes/segundo. Argon2id/bcrypt são propositalmente lentos, salted e com work factor ajustável"
      - "SHA-256 não suporta strings UTF-8 em todos os locales"
      - "SHA-256 é deprecated por NIST desde 2020"
    correct: 1
    explanation: "Hashing de senha exige slow + salted + work factor ajustável. SHA-256 puro é rápido (GPU faz bilhões/s), permitindo rainbow tables e brute force massivo. Argon2id é o vencedor PHC e recomendação OWASP atual."
  - q: "Qual a maior diferença prática entre OAuth2 e OIDC?"
    options:
      - "OIDC é mais antigo que OAuth2 e foi substituído"
      - "OAuth2 é framework de autorização delegada (acesso a recursos); OIDC é uma camada sobre OAuth2 que adiciona identidade via ID token (JWT), permitindo authn correta"
      - "OIDC só funciona com Google; OAuth2 funciona com qualquer provider"
      - "OAuth2 inclui MFA built-in; OIDC não"
    correct: 1
    explanation: "OAuth2 foi desenhado para autorização (deixar app B acessar recurso em provider A), não autenticação. OIDC adiciona ID token com claims de identidade (sub, email), formalizando 'login com X'."
  - q: "Qual JWT pitfall consiste em o atacante mudar o `alg` no header e o server aceitar a forja?"
    options:
      - "Replay attack via `iat` no passado"
      - "Key confusion: token com `alg: HS256` mas server validando com chave pública RSA como segredo HMAC, deixando o atacante assinar tokens com a public key"
      - "Token expirado aceito por bug de comparação de string"
      - "JWT em URL acabar em logs"
    correct: 1
    explanation: "Em key confusion, o atacante muda `alg` para HS256 e usa a public key RSA do server como segredo HMAC. Libs antigas validavam sem checar o alg esperado. Sempre fixe o alg server-side."
  - q: "No fluxo OAuth2 Authorization Code com PKCE, qual o papel do `code_verifier`?"
    options:
      - "É o segredo que o cliente envia ao endpoint /authorize para autenticar a request"
      - "É um valor aleatório gerado pelo cliente; o hash (`code_challenge`) vai na auth request, e o verifier original é enviado na troca code→token, mitigando interceptação do code"
      - "É a string usada para criptografar o ID token em trânsito"
      - "É o nonce que previne replay de ID tokens já consumidos"
    correct: 1
    explanation: "PKCE protege Authorization Code contra interceptação: cliente gera `code_verifier` aleatório, manda `S256(code_verifier)` em /authorize e prova posse enviando `code_verifier` em /token. Atacante que capture o code não consegue trocá-lo por token."
  - q: "Em refresh token rotation com family-based detection, o que acontece se um refresh token já rotacionado for reutilizado?"
    options:
      - "Apenas aquele token é invalidado, os outros continuam ativos"
      - "Toda a family inteira é revogada, indicando replay/roubo, e o usuário é forçado a logar novamente"
      - "É emitido um novo token automaticamente sem alerta"
      - "O server retorna 200 e ignora o uso duplicado por idempotência"
    correct: 1
    explanation: "Reutilizar um refresh já rotacionado é sinal de replay (alguém clonou o token). RFC 9700 recomenda invalidar a family inteira: todos os descendentes do login original são revogados, forçando re-auth."
---
