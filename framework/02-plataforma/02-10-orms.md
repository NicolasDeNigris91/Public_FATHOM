...
status: locked
quiz:
  - q: "Qual é o problema clássico N+1 em ORMs e como detectá-lo?"
    options:
      - "Múltiplas conexões simultâneas; detectado por `pg_stat_activity`"
      - "1 query buscando lista + N queries para relações de cada item; detectado via SQL logger ou APM mostrando dezenas de queries idênticas por request"
      - "Queries com mais de N joins; detectado por EXPLAIN"
      - "Lock em mais de N rows simultaneamente"
    correct: 1
    explanation: "N+1 é clássico: lista de N orders + 1 query por order para buscar courier = 1+N queries. Detectado em logger SQL, pg_stat_statements ou APM (Datadog mostra dezenas de spans idênticos)."
  - q: "Em uma transação Drizzle/Prisma, qual o erro comum a evitar?"
    options:
      - "Usar `await` dentro do callback"
      - "Usar o client global em vez do `tx` passado para o callback, fazendo queries escaparem da transação"
      - "Fazer mais de 10 queries dentro do mesmo bloco"
      - "Usar TypeScript estrito"
    correct: 1
    explanation: "O callback de transação recebe um `tx`. Funções internas devem usar `tx`, não o `db` global — senão queries vão fora da transação, sem atomicidade nem rollback."
  - q: "Em pgbouncer modo `transaction`, por que prepared statements falham silenciosamente?"
    options:
      - "Pgbouncer não suporta SQL"
      - "Cada transação pega conexão diferente do pool, e o statement preparado em uma conexão não persiste em outra; sintoma é `prepared statement 's_1' does not exist` intermitente"
      - "Prepared statements são deprecated em Postgres 16"
      - "Apenas funciona em modo session"
    correct: 1
    explanation: "Em transaction mode, conexões rotacionam entre transações. Prepared statements (que vivem na sessão) deixam de existir na conexão seguinte. Mitigação: session mode, Prisma Accelerate, Hyperdrive, ou desabilitar prepare no driver."
  - q: "Em uma migration zero-downtime, qual o padrão correto para renomear `payout_cents` para `payout_value`?"
    options:
      - "`ALTER TABLE orders RENAME COLUMN payout_cents TO payout_value` em uma única migration"
      - "Expand-Migrate-Contract em 3 releases: adicionar nova coluna nullable + dual-write, backfill async + flip reads, drop old column"
      - "Criar nova tabela e copiar todos os dados"
      - "Apenas em janela de manutenção"
    correct: 1
    explanation: "RENAME quebra app code que ainda lê o nome antigo. Padrão zero-downtime: Expand (add nova coluna nullable, dual-write), Migrate (backfill via job, flip reads), Contract (drop old). Cada release rollback-safe."
  - q: "Sobre DataLoader pattern, qual regra é crítica para evitar bugs?"
    options:
      - "Criar instância global compartilhada entre requests para reuso"
      - "Criar 1 instance por request; o output array deve ter mesmo tamanho e MESMA ORDEM dos input ids"
      - "Usar apenas em GraphQL, nunca em REST"
      - "Sempre usar `maxBatchSize: Infinity`"
    correct: 1
    explanation: "DataLoader cache deve ser per-request (compartilhar entre requests vaza dados cross-tenant). E o array de saída precisa ter mesmo tamanho e mesma ordem dos ids — use Map + .map(id => map.get(id)) para garantir."
---
