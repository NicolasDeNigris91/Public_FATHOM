...
Este módulo é **a teoria de baixo de tudo**: o que é uma race condition formal, o que é happens-before, o que memory barriers fazem, por que `volatile` em Java não é o mesmo que `volatile` em C, como mutexes funcionam por baixo (futex no Linux), o que é lock-free, o que é wait-free, ABA problem, MESI, false sharing, e por que `Atomic<T>` não basta. Sem isso, A07 (Node event loop), A09 (Postgres MVCC), P11 (Go/Rust concurrency), S04 (resilience) viram cargo cult.
...
- Liga com **N02** (OS): futex, context switch, scheduler.
- Liga com **N06** (paradigms): actors, CSP, STM como alternativas a shared memory.
- Liga com **N07** (JS): event loop como modelo concorrente; Workers + Atomics.
- Liga com **A07** (Node internals): libuv pool, race entre callbacks.
- Liga com **A09** (Postgres MVCC): isolation = forma de ordering em DB.
- Liga com **A11** (Redis): single-threaded design escapa de muito disso.
- Liga com **P10** (backend perf): contention é gargalo principal em alta concorrência.
- Liga com **P11** (Go/Rust): goroutines + channels (CSP), Send/Sync em Rust.
- Liga com **S04** (resilience): backpressure, bulkheads são patterns concorrentes.
- Liga com **S14** (Formal Methods): TLA+ modela concorrência formalmente.
