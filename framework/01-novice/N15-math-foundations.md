...
# N15 — Math Foundations for Computing
...
- N12 (cripto): você não entende discrete log nem por que Curve25519 é segura.
- S10 (AI/LLM): vector embeddings, gradient descent, attention — opacos.
- S01 (distributed): probabilistic guarantees, FLP, Byzantine, gossip — só intuição.
- A15 (search): TF-IDF, BM25 são fórmulas, HyperLogLog é probabilístico.
- N13 (compilers): type systems usam logic; correção via teorema.
- S14 (formal methods): impossível.
...
Aplicações: A/B tests (S16), anomaly detection, data quality.

### 2.13 Big-O e crescimento

(Reforço de N05 com mais rigor)
...
### 2.15 Cryptographic math (preview de N12)
...
- Use t-digest (P13) ou HDR Histogram pra estimar percentis em streaming.
...
**Bloom filter**: cobertos em N04 §2.15.
...
- Liga com **N04** (data structures): graph, hash, probabilistic.
- Liga com **N05** (algorithms): probabilistic, randomized.
- Liga com **N12** (cripto): número, EC, mod arithmetic.
- Liga com **N13** (compilers): graph (CFG, register alloc).
- Liga com **A09** (Postgres): query stats, percentiles.
- Liga com **A15** (search): TF-IDF, BM25, vector cosine.
- Liga com **A16** (graph DBs): graph theory.
- Liga com **P10** (backend perf): probability of latency tail.
- Liga com **P13** (analytical DBs): t-digest, HLL.
- Liga com **S01** (distributed): probability of failures, gossip.
- Liga com **S04** (resilience): jitter, exponential backoff statistics.
- Liga com **S10** (AI/LLM): linear algebra is core.
- Liga com **S11** (Web3): cripto + Merkle.
- Liga com **S14** (formal methods): logic.
