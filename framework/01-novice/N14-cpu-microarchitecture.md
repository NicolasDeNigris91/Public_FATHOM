...
### 2.10 Memory ordering (revisita N11)
...
Implicação: thread A store, thread B load same address — A pode estar no store buffer, B vê stale do cache. (Origem do exemplo §2.2 N11.)
...
- **Avoid false sharing** (já visto em N11).
...
3. **Bench false sharing** (revisita N11):
...
- Liga com **N01** (computation model): camada acima.
- Liga com **N02** (OS): kernel, scheduler, paging.
- Liga com **N04** (data structures): cache-friendly choices.
- Liga com **N05** (algorithms): real complexity inclui cache.
- Liga com **N11** (concurrency): memory model x86 vs ARM.
- Liga com **A07** (Node internals): V8 hidden classes, hot/cold path.
- Liga com **P10** (backend perf): profiling em produção.
- Liga com **P11** (Go/Rust): zero-cost abstractions matter.
- Liga com **P12** (Wasm): SIMD, threads.
- Liga com **P13** (analytical DBs): vectorized engines.
- Liga com **S10** (AI/LLM): inference perf.
