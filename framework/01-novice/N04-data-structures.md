...
E quando você for estudar Postgres (A09), B-Trees são índice. Quando estudar Redis (A11), Skip Lists são sorted sets. Quando estudar Kafka (S02), particionamento usa hashing consistente. **Estruturas de dados estão em todo lugar do stack moderno.**
...
Algoritmos clássicos vivem aqui (BFS, DFS, Dijkstra, A*) — ver [N05](N05-algorithms.md).
...
Conexão com N14 (CPU microarch): cache miss custa 100-300 ciclos. Layout matters.
...
**Encerramento:** após N04, escolha de estrutura deixa de ser intuição e vira raciocínio. Você nunca mais vai usar `Array.shift` pra implementar fila.
