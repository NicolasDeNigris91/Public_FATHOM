...
- Liga com **N04** (data structures): trees, hash maps, scope chains.
- Liga com **N05** (algorithms): graph coloring (register alloc), dataflow analysis.
- Liga com **N06** (paradigms): functional core via lambda + immutability.
- Liga com **N07** (JS): V8 = parser + bytecode interp + JIT. Hidden classes.
- Liga com **N08** (TS): você acabou de escrever um type checker em TS.
- Liga com **A04** (React): JSX é sintaxe não-padrão; Babel/SWC parseia.
- Liga com **A10** (ORMs): query builders são DSLs internas; geração de SQL é codegen.
- Liga com **P11** (Go/Rust): rustc usa LLVM IR; Go tem compilador próprio.
- Liga com **P12** (Wasm): target de codegen em vários compiladores modernos.
- Liga com **S05** (API): GraphQL e Protobuf têm parsers/codegen.
