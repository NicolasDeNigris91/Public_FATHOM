...
stage: novice
prereqs: [N01, N02, N03, N04, N05, N06, N07, N08, N09, N10]
status: locked
gates:
  pratico: { status: pending, date: null, attempts: 0, notes: null }
---

# CAPSTONE Novice — HTTP/1.1 Server From Scratch

## 1. Por que esse capstone existe

Ao final do Novice você implementou pequenos exercícios isolados (LRU cache, MyPromise, mygit, mini-shell, etc). O capstone integra **tudo num projeto único de produção-grade simplificado**: um servidor HTTP/1.1 que vai do TCP cru até resposta a clientes reais como `curl` e `wrk`.

O que ele força você a integrar:
- **Sistemas (N02):** sockets como FDs, `accept()` em loop, gestão de muitas conexões.
- **Redes (N03):** parsing manual de HTTP/1.1 (request line, headers, body, chunked encoding, keep-alive).
- **Estruturas (N04):** LRU cache (hash + DLL), table de routers (trie ou hash), pool de buffers.
- **Algoritmos (N05):** matching de paths (string algorithms básicos), parser baseado em state machine.
- **Paradigmas (N06):** state machine clara, error handling com `Result`-like.
- **JS profundo (N07):** event loop, streams, backpressure, microtasks.
- **TypeScript (N08):** tipos estritos pra request/response, types pra parser states.
- **Git (N09):** repo separado bem versionado.
- **Unix (N10):** logging estruturado, CLI, signals (SIGTERM graceful shutdown).
...
3. **Middlewares** (estilo Express):
   ...
   - Order matters.
4. **Static file serving** com **LRU cache** (~64 MB, usa o LRUCache do desafio N04):
...
- **Routing avançado** com **trie** de paths (ver N04).
...
Esse é também o **fundamento** pro próximo capstone (Apprentice — Logística v1), onde a aplicação vai usar HTTP via libs maduras, mas você já saberá o que cada coisa significa.
