...
Este módulo te dá o JS de verdade. Depois dele, **TypeScript ([N08](N08-typescript-type-system.md))** vira evolução natural.
...
- **Conecta com [N02 — OS](N02-operating-systems.md):** event loop é construído sobre `epoll`/`kqueue`/`IOCP` via libuv. Microtasks rodam em user space; macrotasks são frequentemente acionadas por syscalls completas.
...
- **Effective TypeScript** (Vanderkam) — pra A8 mas relevante.
...
**Encerramento:** N07 é o módulo que **separa** quem escreve JS de quem **entende** JS. Depois dele, frameworks deixam de ser caixas mágicas. Você passa a ler código de libs, debug em DevTools, e otimizar V8 com base em sinais reais.
