...
Você não precisa **inventar** primitives — isso é missão de criptógrafos. Mas precisa entender o **que cada primitive garante**, **quando usar qual**, **quais são os modos de falha**. Sem essa base, A13 (Auth — JWT/OAuth2/OIDC), P08 (OWASP), S11 (Web3) viram cargo cult e você vai escrever código inseguro com confiança.
...
### 2.11 TLS 1.3 (overview, integra com N03)
...
- Liga com **N03** (networking): TLS 1.3 usa ECDHE, AEAD, certificates.
- Liga com **N04** (data structures): Merkle trees, Bloom filters criptográficos.
- Liga com **N09** (Git internals): SHA-1 (depreciado), SHA-256 transition.
- Liga com **A13** (Auth): senhas, JWT, OAuth2, OIDC dependem disso.
- Liga com **P08** (OWASP): A02 cryptographic failures.
- Liga com **S05** (API design): mTLS entre serviços.
- Liga com **S11** (Web3): hash, signatures, Merkle, BLS — toda a base.
