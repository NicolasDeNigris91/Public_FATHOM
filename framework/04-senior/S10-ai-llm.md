...
Aplicar resilience patterns (S04): timeouts, retries em transient errors, circuit breaker, idempotency keys, fallback (modelo menor ou cached response).
...
- Liga com **A09** (Postgres): pgvector pra embeddings.
- Liga com **A14** (real-time): SSE streaming.
- Liga com **P05** (AWS): Bedrock pra LLMs managed.
- Liga com **P07** (observability): trace de cada call, cost dashboards.
- Liga com **P08** (security): prompt injection, PII masking.
- Liga com **S04** (resilience): retries, fallback, circuit breaker em provider.
- Liga com **S05** (API design): tool calling = API design problem.
- Liga com **S12** (tech leadership): AI roadmap, eval-driven dev culture.
