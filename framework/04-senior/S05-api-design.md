...
9. **gRPC interno**:
   - Routing engine (P11 Rust) exposto via gRPC.
...
- Liga com **A08** (frameworks): plugins, schema, error handling.
- Liga com **A13** (auth): JWT em headers, scope-based authz.
- Liga com **P04** (CI/CD): Spectral lint, contract tests.
- Liga com **P07** (observability): trace request through API gateway.
- Liga com **P08** (security): OWASP API top 10 já tocou.
- Liga com **S02** (messaging): webhooks são event-driven outbound.
- Liga com **S03** (event-driven): events públicos vs internos; AsyncAPI.
- Liga com **S06** (DDD): bounded context = API boundary.
- Liga com **S08** (services): per-service vs gateway BFF.
