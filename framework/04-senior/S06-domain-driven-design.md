...
5. **Cross-context comunication**:
   ...
   - Integration events (em broker, S02/S03) só onde cross-deploy.
...
7. **Strategic decisions documented**:
   - `ARCHITECTURE.md` com:
     ...
     - Decisões: por que Routing é candidato a serviço separado (P11 Rust)? Por que Auth é generic (poderia trocar Auth0 sem dor)?
...
- 1 bounded context extracted pra serviço separado (Routing → Rust do P11). Demonstre ACL.
- Hexagonal/Ports-Adapters dentro de bounded context.
- Event sourcing in Order context (S03).

---

## 5. Extensões e Conexões

- Liga com **A08** (frameworks): hexagonal app structure.
- Liga com **A09** (Postgres): aggregates → tables; transactions per aggregate.
- Liga com **S03** (event-driven): domain events ↔ integration events.
- Liga com **S05** (API): API = published language do bounded context.
- Liga com **S07** (architectures): hexagonal, clean, onion.
- Liga com **S08** (services vs monolith): bounded context é unidade candidata a serviço.
- Liga com **S12** (tech leadership): DDD requires conversa stakeholders.
