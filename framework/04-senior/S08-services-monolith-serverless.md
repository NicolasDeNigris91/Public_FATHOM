...
# S08 — Services vs Monolith vs Serverless
...
Cross-service ACID = sagas (S03). Não 2PC.
...
EDA (S03) é ortogonal a monolith vs microservices. Você pode ter:
...
2. **Strangler Fig real**:
   - Webhook ingestor (Go, P11) já existe.
...
- Liga com **P02-P05** (containers, K8s, AWS): platforms suportando cada estilo.
- Liga com **P03** (K8s): mesh em microservices.
- Liga com **S01** (theory): consistência cross-service.
- Liga com **S02** (messaging): cross-service comm.
- Liga com **S04** (resilience): bulkhead, circuit breaker entre services.
- Liga com **S06** (DDD): bounded context = candidato a service.
- Liga com **S07** (architectures): style interno vs externa.
- Liga com **S09** (scaling): scale horizontalmente cada deployable.
- Liga com **S12** (tech leadership): Conway's law.
