...
1. **Baseline**:
   - Capture métricas atuais (do P10): throughput, latency p99, recursos.
...
- Liga com **A09** (Postgres): replicas, sharding via Citus.
- Liga com **A11** (Redis): cluster, fan-out.
- Liga com **A14** (real-time): WS scale.
- Liga com **P03** (K8s): HPA, autoscaling.
- Liga com **P05** (AWS): RDS, ElastiCache, Aurora Global, ALB, CloudFront.
- Liga com **P07** (observability): SLO + capacity dashboards.
- Liga com **P10** (perf backend): otimização precede scaling.
- Liga com **S01** (theory): consistency em multi-region.
- Liga com **S04** (resilience): backpressure, bulkhead.
- Liga com **S08** (services): scale per deployable.
