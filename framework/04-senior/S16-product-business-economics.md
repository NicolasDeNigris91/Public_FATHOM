...
# S16 — Product, Business & Unit Economics
...
- **Infra cost**: efficiency de queries, cache, autoscale, spot. P10/P13 ajudam.
...
- **Performance**: P09 mostra Core Web Vitals → conversão.
...
1. **Dashboard de business metrics** (Grafana ou Metabase contra ClickHouse/Timescale, P13 conexão):
...
- Liga com **A18** (payments): take rate, billing, ledger.
- Liga com **P07** (observability): metrics customer-facing.
- Liga com **P10** (backend perf): infra cost direct correlation.
- Liga com **P13** (analytical DBs): warehouse pra BI.
- Liga com **S03** (event-driven): event log = source of truth pra metrics.
- Liga with **S12** (tech leadership): proposals decision-making.
- Liga com **S15** (OSS): sustainability models são variantes destes.
- Liga com **CAPSTONE-senior**: Logística v3 inclui dashboards.
