...
Em sistemas multi-tenant, rate limit por tenant (vimos em A11). Garante 1 tenant não engole capacity.

### 2.8 Load shedding

Quando overloaded, **rejeite** requests "novos" pra preservar SLO em-curso. Vimos em P10.
...
Multi-region active-active complexo (S09). Single-region multi-AZ é base.
...
Game days (P15) executam manualmente; chaos engineering automatiza.

### 2.27 Failure budget aplicado

Erro budget (P15): `1 - SLO`. SLO 99.9% → 0.1% = 43 min/mês.
...
- Liga com **A07** (Node): AbortController, AbortSignal.timeout.
- Liga com **A11** (Redis): rate limit, cache-as-fallback.
- Liga com **P03** (K8s): probes, preStop, PDB.
- Liga com **P07** (observability): SLO, burn rate, dependency health.
- Liga com **P10** (perf backend): load shedding.
- Liga com **S01** (theory): retries em modelos async.
- Liga com **S02** (messaging): consumer retries, DLQ.
- Liga com **S03** (event-driven): saga compensations, outbox retries.
- Liga com **S09** (scaling): bulkheads, multi-region.
